...
let quotas = [
  ...
  { id: 2, unidadId: 2, concepto: 'Privado B', monto: 900, estado: 'PENDIENTE' },
];
let payments = [
  { id: 1, cuotaId: 1, montoPagado: 40, fechaPago: '2026-09-11', referencia: 'ABONO-A' },
...
async function setup(role = 'RESIDENTE', signedIn = false) {
  ...
  await context.route('**/*', async (route) => {
    ...
    if (method === 'POST' || method === 'PUT') writes.push({ path, method, body });
    let result;
    let status = 200;
    if (path === '/auth/register' || path === '/auth/login')
      ...
      const index = residents.findIndex((r) => String(r.id) === path.split('/')[2]);
    ...
    else if (path === '/incidencias' || path === '/reservas') result = [];
    else {
      status = 404;
...
try {
  ...
  await page.getByRole('button', { name: 'Guardar ficha de residente' }).click();
  ...
  await admin.page.getByLabel('Concepto', { exact: true }).fill('Cuota octubre');
  await admin.page.getByLabel('Fecha de emisión').fill('2026-09-11');
  await admin.page.getByLabel('Vencimiento', { exact: true }).fill('2026-10-31');
  ...
  await admin.page.getByRole('button', { name: 'Registrar pago recibido' }).click();
  ...
  await admin.page.screenshot({ path: 'tests/artifacts/admin-payments.png', fullPage: true });
  await admin.context.close();

  profileFailure = true;
  const failed = await setup('RESIDENTE', true);
  await failed.page.goto('http://127.0.0.1:5198/pagos');
  await failed.page.getByText('Servicio temporalmente no disponible', { exact: true }).waitFor();
  assert.equal(await failed.page.getByText('Mantenimiento A', { exact: true }).count(), 0);
